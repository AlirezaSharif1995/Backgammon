...
public class Controller : MonoBehaviour
{
    ...
    private void Update()
    {
        loading = _signController.signing;
        
        SetCanvas(avatarUI, avatar);
        SetCanvas(nicknameUI, nickname);
        SetCanvas(loadingUI, loading);
        SetCanvas(regionUI, region);
        SetCanvas(regionScrollUI, regionScroll);
        SetCanvas(birthdateUI, birthdate);
        SetCanvas(monthScrollUI, monthScroll);
        SetCanvas(dayScrollUI, dayScroll);
        SetCanvas(yearScrollUI, yearScroll);
        SetCanvas(yearScrollUI, yearScroll);
        SetCanvas(boardUI, board);
        SetCanvas(checkerUI, checker);
    ...
    public void UpdateParameter(string label, string data)
    {
        switch (label)
        {
            case "avatarId":
            {
                UpdateData(int.Parse(data), _playerInfo.PlayerData.region, _playerInfo.PlayerData.state, _playerInfo.PlayerData.refrralCode, _playerInfo.PlayerData.birthdate, _playerInfo.PlayerData.nickname);
            }
                break;
            
            case "region":
            {
                UpdateData(_playerInfo.PlayerData.avatarId, data, _playerInfo.PlayerData.state, _playerInfo.PlayerData.refrralCode, _playerInfo.PlayerData.birthdate, _playerInfo.PlayerData.nickname);
            }
                break;
            
            case "state":
            {
                UpdateData(_playerInfo.PlayerData.avatarId, _playerInfo.PlayerData.region, data, _playerInfo.PlayerData.refrralCode, _playerInfo.PlayerData.birthdate, _playerInfo.PlayerData.nickname);
            }
                break;
            
            case "refrralCode":
            {
                UpdateData(_playerInfo.PlayerData.avatarId, _playerInfo.PlayerData.region, _playerInfo.PlayerData.state, data, _playerInfo.PlayerData.birthdate, _playerInfo.PlayerData.nickname);
            }
                break;
            
            case "birthdate":
            {
                UpdateData(_playerInfo.PlayerData.avatarId, _playerInfo.PlayerData.region, _playerInfo.PlayerData.state, _playerInfo.PlayerData.refrralCode, ConvertStringToIntArray(data), _playerInfo.PlayerData.nickname);
            }
                break;
            
            case "nickname":
            {
                UpdateData(_playerInfo.PlayerData.avatarId, _playerInfo.PlayerData.region, _playerInfo.PlayerData.state, _playerInfo.PlayerData.refrralCode, _playerInfo.PlayerData.birthdate, data);
            }
                break;
        }
    ...
    public async Task UpdateData(int avatarId, string region, string state, string refrralCode, int[] birthdate, string nickname)
    {
        await _authentication.SaveDataAsync(_playerInfo.PlayerData.token, avatarId, region, state, refrralCode, birthdate, nickname);
    ...
    bool CheckInput(string type)
    {
        ...
        switch (type)
        {
            case "nickname":
            {
                ...
                {
                    nicknameErrorTextUI.text = "Nickname cannot be empty";
                    ok = false;
                }
                else
                ...
                {
                    ok = false;
                    nicknameErrorTextUI.text = "Nickname cannot be more than 11 characters";
                }
            }
                break;
        }
    ...
    public void UpdateUI()
    {
        ...
        {
            int currentYear = System.DateTime.Now.Year;
            _profileController.ageTextUI.text = "<color=grey> Age: </color>" + "<color=black>" + (currentYear - _playerInfo.PlayerData.birthdate[2]) + "</color>";
        }
        else
    ...
    public void SetCanvas(CanvasGroup canvas, bool active)
    {
